...
- `hooked` in the payload is `unionHooked(hookedRuntimes(...), report.byRuntime)`: the runtimes named in hook files (user level plus every project directory the caller knows) plus every runtime with at least one recorded action in the report window (30 days in the daemon's daily send). The file check alone under-reports: the daemon exits whenever the build on disk changes, so its list of served projects is often empty at send time, and a project can keep its hook in `.claude/settings.json` (checked in, portable command) rather than the `settings.local.json` the installer writes. `yenop status` checks both files.
